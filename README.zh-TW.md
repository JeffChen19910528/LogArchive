...
依照 [`Skill.md`](./Skill.md) 規格開發：安全地封存應用程式／系統日誌，同時保留其完整性，並允許授權人員在需要時將日誌還原為可讀的明文。
...
本專案依照 [`Skill.md`](./Skill.md) 的完整規格文件開發，內容涵蓋系統架構、備份模式、金鑰管理、稽核事件、跨平台需求、測試需求與 Definition of Done 等細節，建議在修改架構前先閱讀該文件。
