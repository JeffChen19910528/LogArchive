...
public static class Strings
{
    ...
    private static readonly Dictionary<string, (string En, string ZhTw)> Table = new()
    {
        ...
        ["menu.prompt.backup_source_title"] = ("Source directory to back up", "要備份的來源目錄"),
        ["menu.prompt.backup_source"] = ("Source directory path: ", "來源目錄路徑："),
        ["menu.prompt.backup_incremental"] = ("Incremental backup (only changed files)?", "是否增量備份（只備份變更過的檔案）？"),
        ["menu.prompt.id"] = ("Backup ID: ", "備份 ID："),
        ["menu.prompt.id_title"] = ("Select a backup", "選擇一個備份"),
        ["menu.prompt.restore_output"] = ("Restore destination directory (leave blank for default): ", "還原目的地目錄（留空使用預設）："),
        ["menu.prompt.restore_overwrite"] = ("Overwrite existing files at the destination?", "是否覆蓋目的地已存在的檔案？"),
        ["menu.prompt.restore_force"] = ("Restore even if hash verification fails?", "即使雜湊驗證失敗仍要還原嗎？"),
        ["menu.prompt.delete_force"] = ("Delete even if the backup is locked?", "即使備份被鎖定仍要刪除嗎？"),
        ["menu.prompt.retention_dry_run"] = ("Dry run only (don't actually delete anything)?", "僅試跑（不會實際刪除任何項目）嗎？"),
        ["menu.prompt.audit_id"] = ("Filter by backup ID: ", "依備份 ID 篩選："),
        ["menu.prompt.audit_id_title"] = ("Filter audit records by backup", "依備份篩選稽核紀錄"),
        ["menu.prompt.repair_delete"] = ("Delete incomplete backup artifacts?", "要刪除未完成的備份檔嗎？"),
        ["menu.error.id_required"] = ("A backup ID is required.", "必須輸入備份 ID。"),
        ["menu.info.no_backups"] = ("No known backups yet - enter an ID manually.", "目前沒有已知的備份，請手動輸入 ID。"),
        ["menu.item.manual_entry"] = ("Enter manually...", "手動輸入..."),
        ["menu.item.use_config_sources"] = ("Use all sources from the config file", "使用設定檔中的全部來源"),
        ["menu.item.audit_all"] = ("Show all (no filter)", "顯示全部（不篩選）"),
        ["menu.yes"] = ("Yes", "是"),
        ["menu.no"] = ("No", "否"),
